...
public class Ponuda {
	...
	public boolean zakupi(String ime, String prezime, String kontakt) throws SQLException {
		...
		// ubacujemo novu osobu
		String sql = "INSERT INTO Osoba (Ime, Prezime, Kontakt) VALUES (?, ?, ?)";
		int idOsobe;
		try(PreparedStatement ps = DBConnection.getConnection().prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)){ // izvlacimo podatak o generisanom id-u
			ps.setString(1, ime);
			ps.setString(2, prezime);
			ps.setString(3, kontakt);
			ps.executeUpdate();
		
			// dohvatamo id te osobe
		...
		// kada imamo id mozemo zakupiti na vec definisani nacin		
